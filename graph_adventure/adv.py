...
#DFT
while len(roomPath) < 500:
    #set current room - player.currentRoom.id already given 
    current_room = player.currentRoom.id
    #check if its unvisited then..
    #loop through exits, if it's empty - add to roomPath
    if current_room not in roomPath:
        #checking exits
        new_exits = player.currentRoom.getExits()
        roomPath[player.currentRoom.id] = {new_exits[i]: '?' for i in range(0, len(new_exits))}
    #next traversal set to empty list
    next_traverse = []
    for checking_direction in roomPath[current_room]:
        #check for unexplored exits ...looking for next direction
        if roomPath[current_room][checking_direction] == '?':
            next_traverse = checking_direction
            #if next_traverse value none - append
            if next_traverse != None:
                traversalPath.append(next_traverse)
                player.travel(next_traverse)
                next_room = player.currentRoom.id
                #checking again for next '?'
                if next_room not in roomPath:
                    next_exits = player.currentRoom.getExits()
                    roomPath[player.currentRoom.id] = {next_exits[x]: '?' for x in range(0, len(next_exits))}
            #ability to go opposite direction
            roomPath[current_room][next_traverse] = next_room
            roomPath[next_room][opposites(next_traverse)] = current_room
            current_room = (next_room)

            break


    #recursion: shortest path 
    pathSearch = bfs(roomPath, current_room)
    #once recursion finds which way to go 
    if pathSearch != None:
        for rooms in pathSearch:
            for which_way in roomPath[current_room]:
                if roomPath[current_room][which_way] == rooms:
                    traversalPath.append(which_way)
                    player.travel(which_way)
            current_room = player.currentRoom.id
